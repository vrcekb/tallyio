...
export const generateRecentTransactions = (count: number): Transaction[] => {
  ...
  return Array(count).fill(null).map((_, i) => ({
    id: `tx-${Date.now()}-${i}`,
    timestamp: format(new Date(Date.now() - Math.random() * 3600000), 'yyyy-MM-dd HH:mm:ss'),
    strategy: strategyNames[Math.floor(Math.random() * strategyNames.length)],
    network: networks[Math.floor(Math.random() * networks.length)],
    status: status[Math.floor(Math.random() * (status.length - 0.7))] as 'success' | 'pending' | 'failed',
    profitLoss: parseFloat((Math.random() * 0.5 - Math.random() * 0.1).toFixed(4)),
  }));
};
